...
import (
    ...
    "github.com/rocket-pool/smartnode-cli/rocketpool/utils/bytes"
...
func (s *Signature) Verify(msg []byte, pub *PublicKey) bool {
    return gobls.Verify(msg, pub.val, s.val)
}

// VerifyAggregate verifies each public key against a message.
// This is vulnerable to rogue public-key attack. Each user must
// provide a proof-of-knowledge of the public key.
func (s *Signature) VerifyAggregate(pubKeys []*PublicKey, msg []byte) bool {
    var keys []*gobls.PublicKey
    for _, v := range pubKeys {
        keys = append(keys, v.val)
    }
    return s.val.VerifyAggregateCommon(keys, msg)
}
