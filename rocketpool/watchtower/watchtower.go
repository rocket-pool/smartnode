...
func (p *WatchtowerProcess) checkMinipools() {
    ...
    activeMinipools, err := minipool.GetActiveMinipoolsByValidatorPubkey(p.p.CM)
    if err != nil {
    ...
    for pubkey, minipoolAddress := range *activeMinipools {
 ...
 * Check minipool
 */
func (p *WatchtowerProcess) checkMinipool(minipoolAddress common.Address, pubkey string, head *beacon.BeaconHead) {
    ...
    if err != nil {
        p.p.Log.Println(errors.New("Error decoding validator pubkey: " + err.Error()))
        return
    }

    // Get validator status
    validator, err := p.p.Beacon.GetValidatorStatus(pubkeyBytes)
    if err != nil {
    ...
    if status == minipool.STAKING && head.Epoch >= validator.ExitEpoch {
        p.txLock.Lock()
    ...
    if status == minipool.LOGGED_OUT && head.Epoch >= validator.WithdrawableEpoch {
        p.txLock.Lock()
        defer p.txLock.Unlock()
        if txor, err := p.p.AM.GetNodeAccountTransactor(); err != nil {
            p.p.Log.Println(err)
