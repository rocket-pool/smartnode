...
func (t *dissolveInvalidCredentials) dissolveMegapoolValidator(validator megapool.ValidatorInfoFromGlobalIndex, expectedWithdrawalCredentials common.Hash) error {
	...
	proof, slotTimestamp, err := services.GetValidatorProof(t.c, t.w, eth2Config, validator.MegapoolAddress, types.ValidatorPubkey(validator.Pubkey))
	if err != nil {
		return fmt.Errorf("error getting validator proof: %w", err)
	}

	// Get the gas limit
	gasInfo, err := megapool.EstimateDissolveWithProof(t.rp, validator.MegapoolAddress, validator.ValidatorId, slotTimestamp, proof, opts)
	if err != nil {
	...
	// Dissolve
	tx, err := megapool.DissolveWithProof(t.rp, validator.MegapoolAddress, validator.ValidatorId, slotTimestamp, proof, opts)
	if err != nil {
		return err
	}
