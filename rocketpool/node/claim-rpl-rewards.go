...
func (t *claimRplRewards) run() error {
    ...
    // Check for rewards
    rewardAmountWei, err := rewards.GetNodeClaimRewardsAmount(t.rp, nodeAccount.Address, nil)
    if err != nil {
        return err
    }
    if rewardAmountWei.Cmp(big.NewInt(0)) == 0 {
        return nil
    }

    // Log
    t.log.Printlnf("%.6f RPL is available to claim...", math.RoundDown(eth.WeiToEth(rewardAmountWei), 6))
    ...
    // Log & return
    t.log.Printlnf("Successfully claimed %.6f RPL in rewards.", math.RoundDown(eth.WeiToEth(rewardAmountWei), 6))
    return nil
