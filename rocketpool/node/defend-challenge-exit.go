...
func (t *defendChallengeExit) defendChallenge(rp *rocketpool.RocketPool, mp megapool.Megapool, validatorId uint32, state *state.NetworkState, validatorPubkey types.ValidatorPubkey, exiting bool, callopts *bind.CallOpts) error {
	...
	validatorProof, slotTimestamp, err := services.GetValidatorProof(t.c, 0, t.w, state.BeaconConfig, mp.GetAddress(), validatorPubkey)
	if err != nil {
	...
	if !exiting {
		// Get the gas limit
		gasInfo, err = megapool.EstimateNotifyNotExitGas(rp, mp.GetAddress(), validatorId, slotTimestamp, validatorProof, opts)
		if err != nil {
			return err
		}
	} else {
		gasInfo, err = megapool.EstimateNotifyExitGas(rp, mp.GetAddress(), validatorId, slotTimestamp, validatorProof, opts)
		if err != nil {
			return err
		}
	}
	...
	if !exiting {
		...
		tx, err = megapool.NotifyNotExit(rp, mp.GetAddress(), validatorId, slotTimestamp, validatorProof, opts)
		if err != nil {
			return err
		}
	} else {
		t.log.Printlnf("Notifying that validator %d is exiting.", validatorId)
		tx, err = megapool.NotifyExit(rp, mp.GetAddress(), validatorId, slotTimestamp, validatorProof, opts)
		if err != nil {
			return err
		}
	}
