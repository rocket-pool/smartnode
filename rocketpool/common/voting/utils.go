...
func GetSnapshotVotingPower(cfg *config.RocketPoolConfig, address common.Address) (float64, error) {
	client := getHttpClientWithTimeout()
	apiDomain := cfg.Smartnode.GetSnapshotApiDomain()
	id := cfg.Smartnode.GetSnapshotID()
	if apiDomain == "" {
		return 0, nil
	}
...
func GetSnapshotProposals(cfg *config.RocketPoolConfig, address common.Address, delegate common.Address, activeOnly bool) ([]*sharedtypes.SnapshotProposal, error) {
	client := getHttpClientWithTimeout()
	apiDomain := cfg.Smartnode.GetSnapshotApiDomain()
	id := cfg.Smartnode.GetSnapshotID()
	if apiDomain == "" {
