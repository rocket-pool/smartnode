...
func checkCollateral(c *cli.Command) (*api.CheckCollateralResponse, error) {
	...
	remainingBorrow := response.EthBorrowedLimit.Sub(response.EthBorrowed)
	remainingBorrow = remainingBorrow.Sub(response.PendingBorrowAmount)
	response.InsufficientCollateral = remainingBorrow.IsNegative()
...
func CheckCollateral(rp *rocketpool.RocketPool, nodeAddress common.Address, opts *bind.CallOpts) (ethBorrowed units.Wei, ethBorrowedLimit units.Wei, pendingBorrowAmount units.Wei, err error) {
	ethBorrowed, err = node.GetNodeETHBorrowed(rp, nodeAddress, opts)
	if err != nil {
		return units.Wei{}, units.Wei{}, units.Wei{}, fmt.Errorf("error getting node's borrowed ETH amount: %w", err)
	}
