...
func initHandler(ctx snroute.WriteContext) {
	derivationPath := ctx.Request.URL.Query().Get("derivationPath")
	if derivationPath == "" {
		derivationPath = ctx.Request.FormValue("derivationPath")
	}
	resp, err := initWalletWithPath(ctx.Command(), derivationPath)
