...
func RegisterRoutes(router *snroute.Router, c *cli.Command) {
	router.Handle(snroute.Read("/api/wallet/status", func(w http.ResponseWriter, r *http.Request) {
		resp, err := getStatus(c)
		response.WriteResponse(w, resp, err)
	}))

	router.Handle(snroute.Write("/api/wallet/set-password", func(w http.ResponseWriter, r *http.Request) {
		password := r.FormValue("password")
		resp, err := setPassword(c, password)
		response.WriteResponse(w, resp, err)
	}))

	router.Handle(snroute.Write("/api/wallet/init", func(w http.ResponseWriter, r *http.Request) {
	...
	router.Handle(snroute.Read("/api/wallet/recovery-status", func(w http.ResponseWriter, r *http.Request) {
		response.WriteResponse(w, &api.KeyRecoveryStatusResponse{Recovery: activeRecovery.status()}, nil)
	}))

	router.Handle(snroute.Write("/api/wallet/recover", func(w http.ResponseWriter, r *http.Request) {
	...
	router.Handle(snroute.Write("/api/wallet/search-and-recover", func(w http.ResponseWriter, r *http.Request) {
		mnemonic := r.FormValue("mnemonic")
	...
	router.Handle(snroute.Read("/api/wallet/test-recover", func(w http.ResponseWriter, r *http.Request) {
		mnemonic := r.FormValue("mnemonic")
		skipRecovery := r.FormValue("skipValidatorKeyRecovery") == "true"
		derivationPath := r.FormValue("derivationPath")
		walletIndex, _ := strconv.ParseUint(r.FormValue("walletIndex"), 10, 64)
	...
	router.Handle(snroute.Read("/api/wallet/test-search-and-recover", func(w http.ResponseWriter, r *http.Request) {
		mnemonic := r.FormValue("mnemonic")
		address := common.HexToAddress(r.FormValue("address"))
		skipRecovery := r.FormValue("skipValidatorKeyRecovery") == "true"
	...
	router.Handle(snroute.Write("/api/wallet/rebuild", func(w http.ResponseWriter, r *http.Request) {
	...
	router.Handle(snroute.Write("/api/wallet/export", func(w http.ResponseWriter, r *http.Request) {
		resp, err := exportWallet(c)
		response.WriteResponse(w, resp, err)
	}))

	router.Handle(snroute.Write("/api/wallet/masquerade", func(w http.ResponseWriter, r *http.Request) {
		address := common.HexToAddress(r.FormValue("address"))
	...
	router.Handle(snroute.Write("/api/wallet/end-masquerade", func(w http.ResponseWriter, r *http.Request) {
		resp, err := endMasquerade(c)
		response.WriteResponse(w, resp, err)
	}))

	router.Handle(snroute.Read("/api/wallet/estimate-gas-set-ens-name", func(w http.ResponseWriter, r *http.Request) {
	...
	router.Handle(snroute.Write("/api/wallet/set-ens-name", func(w http.ResponseWriter, r *http.Request) {
		name := r.FormValue("name")
		opts, err := services.GetNodeAccountTransactorFromRequest(c, r)
		if err != nil {
			response.WriteErrorResponse(w, err)
