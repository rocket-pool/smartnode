...
func verifyPerformanceHandler(ctx snroute.Context) {
	...
	startEpoch, err := parseUint64(ctx.Request, "startEpoch")
	if err != nil {
		response.WriteErrorResponse(ctx.Writer, err)
		return
	}
	endEpoch, err := parseUint64(ctx.Request, "endEpoch")
	if err != nil {
		response.WriteErrorResponse(ctx.Writer, err)
		return
	}
	...
	if raw := ctx.Request.URL.Query().Get("megapoolAddress"); raw != "" {
		megapoolAddr = common.HexToAddress(raw)
	} else if raw := ctx.Request.FormValue("megapoolAddress"); raw != "" {
		megapoolAddr = common.HexToAddress(raw)
	}
	resp, err := verifyPerformance(ctx.Command(), megapoolAddr, validatorId, startEpoch, endEpoch)
