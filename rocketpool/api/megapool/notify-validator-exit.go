...
func canNotifyValidatorExit(c *cli.Context, validatorId uint32) (*api.CanNotifyValidatorExitResponse, error) {
	...
	gasInfo, err := megapool.EstimateNotifyExitGas(rp, megapoolAddress, validatorId, slotTimestamp, proof, opts)
	if err != nil {
		return nil, err
	}
...
func notifyValidatorExit(c *cli.Context, validatorId uint32) (*api.NotifyValidatorExitResponse, error) {
	...
	validatorProof, slotTimetamp, err := services.GetValidatorProof(c, 0, w, eth2Config, megapoolAddress, types.ValidatorPubkey(validatorInfo.Pubkey))
	if err != nil {
		return nil, err
	}
