...
func RegisterRoutes(router *snroute.Router) {
	...
	snroute.Read("/api/megapool/can-exit-deficit", canExitDeficitHandler).RegisterTo(router)
	snroute.Read("/api/megapool/deficit", deficitHandler).RegisterTo(router)
	snroute.Write("/api/megapool/exit-deficit", exitDeficitHandler).RegisterTo(router)
