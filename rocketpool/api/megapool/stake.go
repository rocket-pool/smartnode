...
func canStake(c *cli.Context, validatorId uint64) (*api.CanStakeResponse, error) {
	...
	gasInfo, err := megapool.EstimateStakeGas(rp, megapoolAddress, uint32(validatorId), slotTimestamp, validatorProof, opts)
	if err != nil {
		return nil, err
	}
...
func stake(c *cli.Context, validatorId uint64) (*api.StakeResponse, error) {
	...
	// Stake
	tx, err := megapool.Stake(rp, megapoolAddress, uint32(validatorId), slotTimestamp, validatorProof, opts)
	if err != nil {
		return nil, err
	}
