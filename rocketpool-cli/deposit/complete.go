...
func completeDeposit(c *cli.Context) error {
    ...
    if nodeBalances.EtherWei.Cmp(requiredBalances.EtherWei) < 0 {
        ...
        response := cliutils.Prompt(p.Input, fmt.Sprintf("Node contract requires %.2f ETH to complete deposit, would you like to pay now from your node account? [y/n]", eth.WeiToEth(remainingEtherRequiredWei)), "(?i)^(y|yes|n|no)$", "Please answer 'y' or 'n'")
    ...
    if nodeBalances.RplWei.Cmp(requiredBalances.RplWei) < 0 {
        ...
        response := cliutils.Prompt(p.Input, fmt.Sprintf("Node contract requires %.2f RPL to complete deposit, would you like to pay now from your node account? [y/n]", eth.WeiToEth(remainingRplRequiredWei)), "(?i)^(y|yes|n|no)$", "Please answer 'y' or 'n'")
        if strings.ToLower(response[:1]) == "n" {
            fmt.Println("Deposit not completed")
            return nil
        }
