...
func notifyValidatorExit(c *cli.Context) error {
	...
	if c.IsSet("validator-id") {
		...
		if len(activeValidators) > 0 {
			...
			selected, _ := cliutils.Select("Please select a validator to notify the exit:", options)
	...
	if !(c.Bool("yes") || cliutils.Confirm(fmt.Sprintf("Are you sure you want to notify about the validator id %d exit?", validatorId))) {
