...
func promoteMinipools(minipool string, yes bool) error {
	...
	if minipool == "" {
		...
		selected, _ := prompt.Select("Please select a minipool to promote:", options)
	...
	if !(yes || prompt.Confirm("Are you sure you want to promote %d minipools?", len(selectedMinipools))) {
