...
func installService(c *cli.Context) error {
    ...
    if c.GlobalString("host") == "" {
        fmt.Println("Please restart your shell session to apply updated user permissions.")
    }
...
func stopService(c *cli.Context) error {
    ...
    // Prompt for confirmation
    if !cliutils.Confirm("Are you sure you want to terminate the Rocket Pool service? Any staking minipools will be penalized, chain databases will be deleted, and ethereum nodes will lose sync progress!") {
        fmt.Println("Cancelled.")
        return nil
    }
