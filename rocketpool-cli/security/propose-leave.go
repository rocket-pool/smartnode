...
func proposeRequestLeave(c *cli.Context) error {
	...
	canPropose, err := rp.SecurityCanProposeRequestLeave()
	if err != nil {
		return err
	}
	if !canPropose.CanPropose {
		fmt.Println("Cannot propose a request to leave:")
	...
	// Submit proposal
	response, err := rp.SecurityProposeRequestLeave()
	if err != nil {
		return err
	}

	fmt.Printf("Proposing a request to leave from the security council...\n")
	...
	// Log & return
	fmt.Println("Successfully submitted a request to leave proposal.")
	return nil
