...
func getSettings() error {
	...
	fmt.Printf("\tMin ETH per Lot:          %.6f ETH\n", response.Auction.LotMinimumEthValue.ToEth().InexactFloat64())
	fmt.Printf("\tMax ETH per Lot:          %.6f ETH\n", response.Auction.LotMaximumEthValue.ToEth().InexactFloat64())
	fmt.Printf("\tLot Duration:             %s\n", response.Auction.LotDuration)
	fmt.Printf("\tStarting Price Ratio:     %.2f%%\n", response.Auction.LotStartingPriceRatio.ToEth().InexactFloat64()*100)
	fmt.Printf("\tReserve Price Ratio:      %.2f%%\n", response.Auction.LotReservePriceRatio.ToEth().InexactFloat64()*100)
	fmt.Println()
	...
	fmt.Printf("\tMin Pool Deposit:                   %.6f ETH\n", response.Deposit.MinimumDeposit.ToEth().InexactFloat64())
	fmt.Printf("\tMax Deposit Pool Size:              %.6f ETH\n", response.Deposit.MaximumDepositPoolSize.ToEth().InexactFloat64())
	...
	fmt.Printf("\tDeposit Fee:                        %.2f%%\n", response.Deposit.DepositFee.ToEth().InexactFloat64()*100)
	...
	fmt.Printf("\tInterval Rate:  %.6f\n", response.Inflation.IntervalRate.ToEth().InexactFloat64())
	...
	fmt.Printf("\toDAO Consensus Quorum:                        %.2f%%\n", response.Network.OracleDaoConsensusThreshold.ToEth().InexactFloat64()*100)
	fmt.Printf("\tNode Penalty Quorum:                          %.2f%%\n", response.Network.NodePenaltyThreshold.ToEth().InexactFloat64()*100)
	fmt.Printf("\tPenalty Size:                                 %.2f%%\n", response.Network.PerPenaltyRate.ToEth().InexactFloat64()*100)
	...
	fmt.Printf("\tMin Commission:                               %.2f%%\n", response.Network.MinimumNodeFee.ToEth().InexactFloat64()*100)
	fmt.Printf("\tTarget Commission:                            %.2f%%\n", response.Network.TargetNodeFee.ToEth().InexactFloat64()*100)
	fmt.Printf("\tMax Commission:                               %.2f%%\n", response.Network.MaximumNodeFee.ToEth().InexactFloat64()*100)
	fmt.Printf("\tCommission Demand Range:                      %.6f ETH\n", response.Network.NodeFeeDemandRange.ToEth().InexactFloat64())
	fmt.Printf("\trETH Collateral Target:                       %.2f%%\n", response.Network.TargetRethCollateralRate.ToEth().InexactFloat64()*100)
	fmt.Printf("\tRewards Submission Enabled:                   %t\n", response.Network.IsSubmitRewardsEnabled)
	fmt.Printf("\tNode Commission Share:                        %.2f%%\n", response.Network.NodeCommissionShare.ToEth().InexactFloat64()*100)
	fmt.Printf("\tNode Commission Share Security Council Adder: %.2f%%\n", response.Network.NodeCommissionShareSecurityCouncilAdder.ToEth().InexactFloat64()*100)
	fmt.Printf("\tVoter Share:                                  %.2f%%\n", response.Network.VoterShare.ToEth().InexactFloat64()*100)
	fmt.Printf("\tProtocol DAO Share:                           %.2f%%\n", response.Network.ProtocolDAOShare.ToEth().InexactFloat64()*100)
	fmt.Printf("\tMax Commission Share Security Council Adder:  %.2f%%\n", response.Network.MaxNodeShareSecurityCouncilAdder.ToEth().InexactFloat64()*100)
	fmt.Printf("\tMax rETH balance delta:                       %.2f%%\n", response.Network.MaxRethBalanceDelta.ToEth().InexactFloat64()*100)
	...
	fmt.Printf("\tMax Stake For Voting Power:    %.2f%%\n", response.Node.MaximumStakeForVotingPower.ToEth().InexactFloat64()*100)
	...
	fmt.Printf("\tBond per Proposal:       %.6f RPL\n", response.Proposals.ProposalBond.ToEth().InexactFloat64())
	fmt.Printf("\tBond per Challenge:      %.6f RPL\n", response.Proposals.ChallengeBond.ToEth().InexactFloat64())
	fmt.Printf("\tChallenge Response Time: %s\n", response.Proposals.ChallengePeriod)
	fmt.Printf("\tQuorum:                  %.2f%%\n", response.Proposals.Quorum.ToEth().InexactFloat64()*100)
	fmt.Printf("\tVeto Quorum:             %.2f%%\n", response.Proposals.VetoQuorum.ToEth().InexactFloat64()*100)
	...
	fmt.Printf("\tMember Quorum:         %.2f%%\n", response.Security.MembersQuorum.ToEth().InexactFloat64()*100)
	...
	fmt.Printf("\tUpgrade Veto Quorum:   %.2f%%\n", response.Security.UpgradeVetoQuorum.ToEth().InexactFloat64()*100)
	...
	fmt.Printf("\tPenalty Threshold:                      %.2f%%\n", response.Megapool.PenaltyThreshold.ToEth().InexactFloat64()*100)
