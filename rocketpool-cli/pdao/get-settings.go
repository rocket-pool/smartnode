...
func getSettings() error {
	...
	fmt.Printf("\tMin ETH per Lot:          %.6f ETH\n", units.WeiToEth(response.Auction.LotMinimumEthValue))
	fmt.Printf("\tMax ETH per Lot:          %.6f ETH\n", units.WeiToEth(response.Auction.LotMaximumEthValue))
	fmt.Printf("\tLot Duration:             %s\n", response.Auction.LotDuration)
	fmt.Printf("\tStarting Price Ratio:     %.2f%%\n", units.WeiToEth(response.Auction.LotStartingPriceRatio)*100)
	fmt.Printf("\tReserve Price Ratio:      %.2f%%\n", units.WeiToEth(response.Auction.LotReservePriceRatio)*100)
	fmt.Println()
	...
	fmt.Printf("\tMin Pool Deposit:                   %.6f ETH\n", units.WeiToEth(response.Deposit.MinimumDeposit))
	fmt.Printf("\tMax Deposit Pool Size:              %.6f ETH\n", units.WeiToEth(response.Deposit.MaximumDepositPoolSize))
	...
	fmt.Printf("\tDeposit Fee:                        %.2f%%\n", units.WeiToEth(response.Deposit.DepositFee)*100)
	...
	fmt.Printf("\tInterval Rate:  %.6f\n", units.WeiToEth(response.Inflation.IntervalRate))
	...
	fmt.Printf("\toDAO Consensus Quorum:                        %.2f%%\n", units.WeiToEth(response.Network.OracleDaoConsensusThreshold)*100)
	fmt.Printf("\tNode Penalty Quorum:                          %.2f%%\n", units.WeiToEth(response.Network.NodePenaltyThreshold)*100)
	fmt.Printf("\tPenalty Size:                                 %.2f%%\n", units.WeiToEth(response.Network.PerPenaltyRate)*100)
	...
	fmt.Printf("\tMin Commission:                               %.2f%%\n", units.WeiToEth(response.Network.MinimumNodeFee)*100)
	fmt.Printf("\tTarget Commission:                            %.2f%%\n", units.WeiToEth(response.Network.TargetNodeFee)*100)
	fmt.Printf("\tMax Commission:                               %.2f%%\n", units.WeiToEth(response.Network.MaximumNodeFee)*100)
	fmt.Printf("\tCommission Demand Range:                      %.6f ETH\n", units.WeiToEth(response.Network.NodeFeeDemandRange))
	fmt.Printf("\trETH Collateral Target:                       %.2f%%\n", units.WeiToEth(response.Network.TargetRethCollateralRate)*100)
	fmt.Printf("\tRewards Submission Enabled:                   %t\n", response.Network.IsSubmitRewardsEnabled)
	fmt.Printf("\tNode Commission Share:                        %.2f%%\n", units.WeiToEth(response.Network.NodeCommissionShare)*100)
	fmt.Printf("\tNode Commission Share Security Council Adder: %.2f%%\n", units.WeiToEth(response.Network.NodeCommissionShareSecurityCouncilAdder)*100)
	fmt.Printf("\tVoter Share:                                  %.2f%%\n", units.WeiToEth(response.Network.VoterShare)*100)
	fmt.Printf("\tProtocol DAO Share:                           %.2f%%\n", units.WeiToEth(response.Network.ProtocolDAOShare)*100)
	fmt.Printf("\tMax Commission Share Security Council Adder:  %.2f%%\n", units.WeiToEth(response.Network.MaxNodeShareSecurityCouncilAdder)*100)
	fmt.Printf("\tMax rETH balance delta:                       %.2f%%\n", units.WeiToEth(response.Network.MaxRethBalanceDelta)*100)
	...
	fmt.Printf("\tMax Stake For Voting Power:    %.2f%%\n", units.WeiToEth(response.Node.MaximumStakeForVotingPower)*100)
	...
	fmt.Printf("\tMember Quorum:         %.2f%%\n", units.WeiToEth(response.Security.MembersQuorum)*100)
	...
	fmt.Printf("\tUpgrade Veto Quorum:   %.2f%%\n", units.WeiToEth(response.Security.UpgradeVetoQuorum)*100)
	...
	fmt.Printf("\tPenalty Threshold:                      %.2f%%\n", units.WeiToEth(response.Megapool.PenaltyThreshold)*100)
