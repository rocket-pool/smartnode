...
func promptTimezone(input *os.File, output *os.File) string {
    ...
    if timezone != "" {
        response := cliutils.Prompt(input, output, fmt.Sprintf("Your system timezone is '%s', would you like to register using this timezone? [y/n]", timezone), "(?i)^(y|yes|n|no)$", "Please answer 'y' or 'n'")
