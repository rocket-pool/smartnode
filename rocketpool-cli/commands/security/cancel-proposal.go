...
func cancelProposal(c *cli.Context) error {
	...
	// Build the TX
	response, err := rp.Api.Security.CancelProposal(selectedProposal.ID)
	if err != nil {
		return err
	}
	...
	if !response.Data.CanCancel {
		...
		if response.Data.NotOnSecurityCouncil {
			fmt.Println("You are not on the Security Council.")
		}
		if response.Data.DoesNotExist {
			fmt.Println("The proposal does not exist.")
		}
		if response.Data.InvalidProposer {
			fmt.Println("You did not make this proposal.")
		}
