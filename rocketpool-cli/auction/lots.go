...
func getLots() error {
	...
	for status := 0; status < 2; status++ {
		...
		for _, lot := range statusLots {
			...
			fmt.Printf("RPL starting price:   %.6f\n", math.RoundDown(math.WeiToEth(lot.Details.StartPrice), 6))
			fmt.Printf("RPL reserve price:    %.6f\n", math.RoundDown(math.WeiToEth(lot.Details.ReservePrice), 6))
			fmt.Printf("RPL current price:    %.6f\n", math.RoundDown(math.WeiToEth(lot.Details.CurrentPrice), 6))
			fmt.Printf("Total RPL amount:     %.6f\n", math.RoundDown(math.WeiToEth(lot.Details.TotalRPLAmount), 6))
			fmt.Printf("Claimed RPL amount:   %.6f\n", math.RoundDown(math.WeiToEth(lot.Details.ClaimedRPLAmount), 6))
			fmt.Printf("Remaining RPL amount: %.6f\n", math.RoundDown(math.WeiToEth(lot.Details.RemainingRPLAmount), 6))
			fmt.Printf("Total ETH bid:        %.6f\n", math.RoundDown(math.WeiToEth(lot.Details.TotalBidAmount), 6))
			fmt.Printf("ETH bid by node:      %.6f\n", math.RoundDown(math.WeiToEth(lot.Details.AddressBidAmount), 6))
			if lot.Details.Cleared {
	...
	if len(claimableLots) > 0 {
		...
		for _, lot := range claimableLots {
			fmt.Printf("- lot %d (%.6f ETH bid @ %.6f ETH per RPL)\n", lot.Details.Index, math.RoundDown(math.WeiToEth(lot.Details.AddressBidAmount), 6), math.RoundDown(math.WeiToEth(lot.Details.CurrentPrice), 6))
		}
		fmt.Println("")
	}
	if len(biddableLots) > 0 {
		...
		for _, lot := range biddableLots {
			fmt.Printf("- lot %d (%.6f RPL available @ %.6f ETH per RPL)\n", lot.Details.Index, math.RoundDown(math.WeiToEth(lot.Details.RemainingRPLAmount), 6), math.RoundDown(math.WeiToEth(lot.Details.CurrentPrice), 6))
		}
		fmt.Println("")
	}
	if len(recoverableLots) > 0 {
		...
		for _, lot := range recoverableLots {
			fmt.Printf("- lot %d (%.6f RPL unclaimed)\n", lot.Details.Index, math.RoundDown(math.WeiToEth(lot.Details.RemainingRPLAmount), 6))
		}
		fmt.Println("")
	}
