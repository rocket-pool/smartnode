...
func createDoppelgangerStep(wiz *wizard, currentStep int, totalSteps int) *choiceWizardStep {
	...
	show := func(modal *choiceModalLayout) {
		...
		if buttonIndex == 1 {
			wiz.md.Config.ValidatorClient.VcCommon.DoppelgangerDetection.Value = true
		} else {
			wiz.md.Config.ValidatorClient.VcCommon.DoppelgangerDetection.Value = false
		}
	...
	return newChoiceStep(
		...
		[]string{},
		76,
		"Validator Client > Doppelganger Protection",
