...
import (
	...
	"github.com/rocket-pool/smartnode/rocketpool-daemon/common/beacon/client"
...
func NewBeaconClientManager(cfg *config.RocketPoolConfig) (*BeaconClientManager, error) {
	...
	if cfg.IsNativeMode {
		...
		primaryProvider = fmt.Sprintf("http://%s:%d", docker.ContainerName_BeaconNode, cfg.ConsensusCommon.ApiPort.Value.(uint16))
