...
func NewExecutionClientManager(cfg *config.RocketPoolConfig) (*ExecutionClientManager, error) {
	...
	if cfg.IsNativeMode {
		...
		primaryEcUrl = fmt.Sprintf("http://%s:%d", docker.ContainerName_ExecutionClient, cfg.ExecutionCommon.HttpPort.Value)
	} else {
...
func (p *ExecutionClientManager) CheckStatus(cfg *config.RocketPoolConfig) *api.ClientManagerStatus {
	...
	if status.FallbackEnabled {
		...
		if status.FallbackClientStatus.Error == "" && status.FallbackClientStatus.NetworkId != expectedChainID {
			...
			status.FallbackClientStatus.Error = fmt.Sprintf("The fallback client is using a different chain [%s%s%s, Chain ID %d] than what your node is configured for [%s, Chain ID %d]", terminal.ColorYellow, getNetworkNameFromId(status.FallbackClientStatus.NetworkId), terminal.ColorReset, status.FallbackClientStatus.NetworkId, getNetworkNameFromId(expectedChainID), expectedChainID)
			return status
		}
	}
