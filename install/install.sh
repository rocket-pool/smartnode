...
install() {
    ...
    PACKAGE_NAME="smartnode-install.tar.xz"
    if [ "$PACKAGE_VERSION" = "latest" ]; then
        PACKAGE_URL="https://github.com/rocket-pool/smartnode/releases/latest/download/$PACKAGE_NAME"
    else
        PACKAGE_URL="https://github.com/rocket-pool/smartnode/releases/download/$PACKAGE_VERSION/$PACKAGE_NAME"
    fi
    ...
    progress 5 "Downloading Smart Node package files..."
    if [ -z "$LOCAL_PACKAGE" ]; then
        { curl -L "$PACKAGE_URL" | tar -xJ -C "$TEMPDIR" || fail "Could not download and extract the Smart Node package files."; } >&2
    else
        if [ ! -f $PACKAGE_NAME ]; then
            fail "Installer package $PACKAGE_NAME does not exist." >&2
        fi
        { tar -f "$PACKAGE_NAME" -xJ -C "$TEMPDIR" || fail "Could not extract the local Smart Node package files."; } >&2
    fi
    { test -d "$PACKAGE_FILES_PATH" || fail "Could not extract the Smart Node package files."; } >&2
