...
- Use `rocketpool service daemon-logs` with `api`, `tasks`, and/or `watchtower` to view the new logs respectively.
...
### Installing Production Releases via the Package Manager
...
- There's a new `Logging` section in the service config TUI, and some new log commands
    - Use `rocketpool service daemon-logs api` to view the new API logs
    - Use `rocketpool service daemon-logs tasks` to view the daemon task loop logs (previously `rocketpool service logs node`)
    - Use `rocketpool service daemon-logs watchtower` to view the watchtower task loop logs (`rocketpool service logs watchtower`)
