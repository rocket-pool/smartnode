...
func TestFeeDisplay(t *testing.T) {
    ...
    if messages, err := testapp.CheckOutput(output.Name(), []string{}, map[int][]string{
        1: []string{"(?i)^The current Rocket Pool user fee paid to node operators is \\d\\.\\d\\d% of rewards$", "Current fee message incorrect"},
        3: []string{"(?i)^The current Rocket Pool user fee paid to node operators is \\d\\.\\d\\d% of rewards$", "Current fee message incorrect"},
        2: []string{"(?i)^The target Rocket Pool user fee to vote for is not set$", "Target fee not set message incorrect"},
        4: []string{"(?i)^The target Rocket Pool user fee to vote for is \\d\\.\\d\\d% of rewards$", "Target fee value message incorrect"},
    }); err != nil {
        t.Fatal(err)
    } else {
