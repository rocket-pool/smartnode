...
// (from Prysm v1.0.0-alpha: /shared/keystore/keystore.go)
//
...
func RetrievePubKey(directory string, password string) (*PublicKey, error) {
	ks := Store{
...
func DecryptKey(keyjson []byte, password string) (*Key, error) {
	...
	secretKey, err := SecretKeyFromBytes(keyBytes)
	if err != nil {
		return nil, err
	}
