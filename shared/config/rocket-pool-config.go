...
func (cfg *RocketPoolConfig) GenerateEnvironmentVariables() map[string]string {
	...
	if cfg.ExecutionClientMode.Value.(config.Mode) == config.Mode_Local {
		...
		envVars["EC_HTTP_ENDPOINT"] = fmt.Sprintf("http://%s:%d", docker.ContainerName_ExecutionClient, cfg.ExecutionCommon.HttpPort.Value)
		envVars["EC_WS_ENDPOINT"] = fmt.Sprintf("ws://%s:%d", docker.ContainerName_ExecutionClient, cfg.ExecutionCommon.WsPort.Value)
		envVars["EC_ENGINE_ENDPOINT"] = fmt.Sprintf("http://%s:%d", docker.ContainerName_ExecutionClient, cfg.ExecutionCommon.EnginePort.Value)
		envVars["EC_ENGINE_WS_ENDPOINT"] = fmt.Sprintf("ws://%s:%d", docker.ContainerName_ExecutionClient, cfg.ExecutionCommon.EnginePort.Value)
	...
	if cfg.ConsensusClientMode.Value.(config.Mode) == config.Mode_Local {
		...
		envVars["CC_API_ENDPOINT"] = fmt.Sprintf("http://%s:%d", docker.ContainerName_BeaconNode, cfg.ConsensusCommon.ApiPort.Value)
		...
		case config.ConsensusClient_Prysm:
			...
			envVars["CC_RPC_ENDPOINT"] = fmt.Sprintf("http://%s:%d", docker.ContainerName_BeaconNode, cfg.Prysm.RpcPort.Value)
		case config.ConsensusClient_Teku:
	...
	if cfg.EnableMevBoost.Value == true {
		...
		if cfg.Smartnode.Network.Value == config.Network_Holesky || cfg.Smartnode.Network.Value == config.Network_Devnet {
			...
			if cfg.MevBoost.Mode.Value == config.Mode_Local {
				...
				envVars[mevBoostUrlEnvVar] = fmt.Sprintf("http://%s:%d", docker.ContainerName_MevBoost, cfg.MevBoost.Port.Value)
