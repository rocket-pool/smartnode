...
const (
	...
	defaultPrysmOpenRpcPort string = string(RPC_Closed)
...
func NewPrysmConfig(cfg *RocketPoolConfig) *PrysmConfig {
	rpcPortModes := PortModes("Allow connections from external hosts. This is safe if you're running your node on your local network. If you're a VPS user, this would expose your node to the internet and could make it vulnerable to MEV/tips theft")
	...
	return &PrysmConfig{
		...
		BnContainerTag: Parameter{
			...
			Default: map[Network]interface{}{
				Network_Mainnet: prysmBnProd,
				Network_Prater:  prysmBnTest,
				Network_Devnet:  prysmBnTest,
				Network_Holesky: prysmBnTest,
			},
			AffectsContainers:  []ContainerID{ContainerID_Eth2},
			CanBeBlank:         false,
...
func (cfg *PrysmConfig) GetParameters() []*Parameter {
	return []*Parameter{
		&cfg.MaxPeers,
		&cfg.RpcPort,
		&cfg.OpenRpcPort,
