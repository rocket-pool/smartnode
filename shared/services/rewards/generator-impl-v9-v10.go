	...
	if checkBeaconPerformance {
		...
		for _, nodeInfo := range r.nodeDetails {
			...
			if nodeInfo.IsEligible && nodeInfo.IsOptedIn && r.elEndTime.After(nodeInfo.OptInTime) {
				eligibleBorrowedEth := nodeInfo.MinipoolEligibleBorrowedEth
				_, percentOfBorrowedEth := r.networkState.GetStakedRplValueInEthAndPercentOfBorrowedEth(eligibleBorrowedEth, nodeInfo.LegacyStakedRpl)
...
func (r *treeGeneratorImpl_v9_v10) calculateNodeBonuses() (*big.Int, error) {
	...
	for _, nsd := range r.nodeDetails {
		...
		eligibleBorrowedEth := nsd.MinipoolEligibleBorrowedEth
		_, percentOfBorrowedEth := r.networkState.GetStakedRplValueInEthAndPercentOfBorrowedEth(eligibleBorrowedEth, nsd.LegacyStakedRpl)
...
func (r *treeGeneratorImpl_v9_v10) calculateNodeRewards() (*big.Int, *big.Int, *big.Int, error) {
	...
	r.log.Printlnf("%s Pool staker ETH before bonuses:    %s (%.3f)", r.logPrefix, poolStakerShareBeforeBonuses.String(), units.NewWei(poolStakerShareBeforeBonuses).ToEth().InexactFloat64())
	r.log.Printlnf("%s Pool staker ETH after bonuses:     %s (%.3f)", r.logPrefix, truePoolStakerAmount.String(), units.NewWei(truePoolStakerAmount).ToEth().InexactFloat64())
	r.log.Printlnf("%s Node Op ETH before bonuses:        %s (%.3f)", r.logPrefix, totalNodeOpShare.String(), units.NewWei(totalNodeOpShare).ToEth().InexactFloat64())
	r.log.Printlnf("%s Node Op ETH after bonuses:         %s (%.3f)", r.logPrefix, totalEthForMinipools.String(), units.NewWei(totalEthForMinipools).ToEth().InexactFloat64())
	...
	for _, attestation := range attestations {
		...
		for _, committeeIndex := range attestation.CommitteeIndices() {
			...
			for position, positionInfo := range rpCommittee.Positions {
				...
				eligibleBorrowedEth := nodeDetails.MinipoolEligibleBorrowedEth
				_, percentOfBorrowedEth := r.networkState.GetStakedRplValueInEthAndPercentOfBorrowedEth(eligibleBorrowedEth, nodeDetails.LegacyStakedRpl)
		...
		for iterationIndex := iterationStartIndex; iterationIndex < iterationEndIndex; iterationIndex++ {
			...
			wg.Go(func() error {
				...
				nodeDetails := &NodeSmoothingDetails{
					...
					LegacyStakedRpl:  big.NewInt(0).Set(nativeNodeDetails.LegacyStakedRPL.BigInt()),
				}
