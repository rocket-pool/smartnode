...
func (c *Client) GetEth2Config() (*Eth2ConfigResponse, error) {
    ...
    // Decode hex data
    if genesisForkVersionBytes, err := hex.DecodeString(hexutil.RemovePrefix(response.GenesisForkVersion)); err != nil {
    ...
    if blsWithdrawalPrefixByteBytes, err := hex.DecodeString(hexutil.RemovePrefix(response.BLSWithdrawalPrefixByte)); err != nil {
...
func (c *Client) GetValidatorStatus(pubkey string) (*ValidatorResponse, error) {
    ...
    // Decode hex data
    if pubkeyBytes, err := hex.DecodeString(hexutil.RemovePrefix(response.Pubkey)); err != nil {
        return nil, errors.New("Error decoding validator pubkey: " + err.Error())
    } else {
        response.PubkeyBytes = pubkeyBytes
    }
    if withdrawalCredentialsBytes, err := hex.DecodeString(hexutil.RemovePrefix(response.Validator.WithdrawalCredentials)); err != nil {
