...
func AssignMaxFeeAndLimit(gasInfo rocketpool.GasInfo, rp *rpsvc.Client, headless bool) error {
	...
	if err != nil {
		...
		return fmt.Errorf("your node has %.6f ETH in its wallet, which is not enough to pay for this transaction with a max fee of %.4f; you require at least %.6f more ETH", eth.WeiToEth(response.Balance), maxFeeGwei, eth.WeiToEth(big.NewInt(0).Sub(ethRequired, response.Balance)))
	}
