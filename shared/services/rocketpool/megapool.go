...
// Check whether we can notify a validator final balance
func (c *Client) CanNotifyFinalBalance(validatorId uint64) (api.CanNotifyFinalBalanceResponse, error) {
	responseBytes, err := c.callAPI(fmt.Sprintf("megapool can-notify-final-balance %d", validatorId))
	if err != nil {
...
// Notify the final balance of a megapool validator
func (c *Client) NotifyFinalBalance(validatorId uint64) (api.NotifyFinalBalanceResponse, error) {
	responseBytes, err := c.callAPI(fmt.Sprintf("megapool notify-final-balance %d", validatorId))
	if err != nil {
