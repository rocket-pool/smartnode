...
func (c *Client) CanNodeDeposit(numValidators uint64, minFee float64, salt *big.Int, numExpressTickets uint32) (api.CanNodeDepositResponse, error) {
	responseBytes, err := c.callAPI(fmt.Sprintf("node can-deposit %d %f %s %d", numValidators, minFee, salt.String(), numExpressTickets))
	if err != nil {
...
// Make a node deposit
func (c *Client) NodeDeposit(numValidators uint64, minFee float64, salt *big.Int, numExpressTickets uint32, submit bool) (api.NodeDepositResponse, error) {
	responseBytes, err := c.callAPI(fmt.Sprintf("node deposit %d %f %s %d %t", numValidators, minFee, salt.String(), numExpressTickets, submit))
	if err != nil {
