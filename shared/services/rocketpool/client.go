...
func (c *Client) compose(composeFiles []string, args string) (string, error) {
	...
	if err := config.WriteEnvFile(tagsPath, cfg.ComposeImageDefaults()); err != nil {
		return "", fmt.Errorf("error writing %s: %w", tagsPath, err)
	}

	composeMain := filepath.Join(expandedConfigPath, runtimeDir, config.ComposeMainFile)
	envFiles := append(cfg.ImagesEnvFiles(), config.ImageTagsEnvFile)
	if err := writeComposeMain(composeMain, expandedConfigPath, envFiles, deployedContainers); err != nil {
		return "", err
	}

	composeFileFlags := []string{
		fmt.Sprintf("-f %s", shellescape.Quote(composeMain)),
	}
	for _, container := range composeFiles {
		composeFileFlags = append(composeFileFlags, fmt.Sprintf("-f %s", shellescape.Quote(container)))
	}

	// Compose interpolates the process environment ahead of include env_file.
	envPrefix := []string{
	...
	for _, key := range overrideKeys {
		envPrefix = append(envPrefix, fmt.Sprintf("%s=%s", key, shellescape.Quote(overrides[key])))
	}
...
// writeComposeMain writes runtime/compose.yml with include + env_file so catalog
// interpolation happens in Compose. Later env files override earlier ones
// (mainnet.env then {network}.env). TUI overrides are set in the process environment.
func writeComposeMain(composeMain, projectDir string, envFiles []string, deployed []string) error {
	var b strings.Builder
	b.WriteString("# Autogenerated - DO NOT MODIFY THIS FILE DIRECTLY\n")
	b.WriteString("include:\n")
	envAbs := make([]string, 0, len(envFiles))
	for _, name := range envFiles {
		envAbs = append(envAbs, filepath.Join(projectDir, name))
	}
	for _, path := range deployed {
		...
		for _, envPath := range envAbs {
			b.WriteString("      - ")
			b.WriteString(envPath)
			b.WriteByte('\n')
		}
	}
	return os.WriteFile(composeMain, []byte(b.String()), 0664)
}
