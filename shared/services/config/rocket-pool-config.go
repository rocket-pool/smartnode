...
func (cfg *RocketPoolConfig) GenerateEnvironmentVariables() map[string]string {
	...
	if cfg.UseFallbackClients.Value == true {
		switch consensusClient {
		case config.ConsensusClient_Prysm:
			AddParametersToEnvVars(cfg.FallbackPrysm.GetParameters(), envVars)
		default:
			AddParametersToEnvVars(cfg.FallbackNormal.GetParameters(), envVars)
		}
	}
	...
	// MEV Boost
	AddParametersToEnvVars(cfg.MevBoost.GetParameters(), envVars)
