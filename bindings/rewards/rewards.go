...
func GetRewardsEvent(rp *rocketpool.RocketPool, index uint64, rocketRewardsPoolAddresses []common.Address, network string, eventCacheDir string, opts *bind.CallOpts) (bool, RewardsEvent, error) {
	// Prefer a cached or GitHub-hosted event file. A miss falls through to the EL log query below.
	data, ok, err := getIntervalRewardsEvent(index, network, eventCacheDir)
	if err != nil {
